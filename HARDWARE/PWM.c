...
void Motor_PWM_Init(void)//直流电机PWM
{
...
//	TIM_OCStructInit(&TIM_OCInitStruct);//给函数赋初始值
	...
	TIM_OC3Init(TIM3,&TIM_OCInitStruct);   //定时器通道初始化
...
//	TIM_OC2PreloadConfig(TIM1,TIM_OCPreload_Enable);
	...
	TIM_ARRPreloadConfig(TIM3,ENABLE);						//使能自动重装载寄存器允许位	
...
void PWM_Servo(u32 servo_Pin,float angle)//舵机旋转角度
{
...
{
	if(DIR==ENCODER_FORWARD)//正转
	{
		TIM_SetCompare3(TIM3,SPEED);
		TIM_SetCompare4(TIM3,0);
	}
	else if(DIR==ENCODER_BACKWARD)//反转
	{
